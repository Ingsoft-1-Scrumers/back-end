...
class CardRepository:
    ...
    @db_session#esta fun en UserRepository
    ...
    def steal_card_from_deck(self, this_user : User) -> dict:
        ...
        card_dict = {'name': random_card.card_name, 
                      'type': random_card.card_type, 
    ...
    @db_session #esta fun en UserRepository
    def get_user_hand(self, user_name: str) -> dict:    #el nombre de usuario es unico?
        user = User.select(name=user_name)
        hand_dict = [{'name': card.card_name, 
                      'type': card.card_type, 
